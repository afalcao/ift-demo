...
/* Insertion must be done at a child of the left most element with
   lowest height in the heap and either left and right children
   availables, or right child available. The inserted element must
   move up afterwards to keep the heap properties. In order to test
   it, you must add height to the RGQNode. The first call must pass
   the top node of the heap. */

int FindPositionForInsertion(RGQueue *Q, int p)
{
  int q1, q2;

  if ((Q->H.elem[p].left == NIL)||(Q->H.elem[p].right == NIL))
    return(p);
  else{
    q1 = FindPositionForInsertion(Q,Q->H.elem[p].left);
    q2 = FindPositionForInsertion(Q,Q->H.elem[p].right);
    if (Q->H.elem[q1].height <= Q->H.elem[q2].height)
      return(q1);
    else
      return(q2);
  }
...
/* Removal requires to move the right most element with highest height
   and no children to the top of the heap and move it down to keep the
   heap properties. The position of this element can be found as
   follows.  */

int FindPositionForRemoval(RGQueue *Q, int p)
{
  int q1=NIL, q2=NIL;
  ...
  if ((Q->H.elem[p].left == NIL)&&(Q->H.elem[p].right == NIL))
    ...
    if (Q->H.elem[p].right != NIL)
      q1 = FindPositionForRemoval(Q,Q->H.elem[p].right);
    if (Q->H.elem[p].left != NIL)
      q2 = FindPositionForRemoval(Q,Q->H.elem[p].left);
